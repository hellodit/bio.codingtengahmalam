<template>
  <div v-if="!pending && product">
    ...
    <div class="max-w-xl mx-auto relative mb-4 pb-2">
      <img :src="product.coverImage" :alt="product.name" class="w-full h-[400px] md:h-[500px] object-cover " />
    </div>
    ...
    <div class="max-w-lg mx-auto space-y-8">
      ...
      <div class="space-y-3">
        ...
        <div class="space-y-2">
          ...
          <!-- Harga dan Diskon -->
          <span v-if="product.price !== undefined && product.price !== null && product.price > 0"
          ...
          <span v-else-if="product.price === 0" class="block text-xl md:text-xl font-bold text-primary mb-1">
            GRATIS
          </span>
        </div>
      ...
      <div
        class="fixed bottom-0 left-0 right-0 z-50 border-t border-border shadow-lg bg-white dark:bg-gray-800 backdrop-blur-sm bg-opacity-95 dark:bg-opacity-95">
        <div class="max-w-lg mx-auto px-4 py-4">
          <div class="grid grid-cols-[1fr_3fr] gap-3">
            <UButton variant="outline" icon="i-lucide-message-circle-more" size="xl" class="w-full justify-center"
              to="https://instagram.com/codingtengahmalam">
            </UButton>

            <!-- CTA Button: Beli Sekarang -->
            <UButton color="primary" variant="solid" :to="product.url" size="xl" trailing-icon="i-heroicons-arrow-right"
              aria-label="Beli Sekarang" class="">
              <p class="py-1 text-center w-full">
                Beli Sekarang
              </p>
            </UButton>
          </div>
          <p v-if="product.ctaHelperText || product.bonus"
